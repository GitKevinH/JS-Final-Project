...
async function getCurrencySymbols() {
  let currencySymbols = await getAPI(
    "https://api.apilayer.com/exchangerates_data/symbols"
  ); // takes posts from URL, passes it to
...
async function addSymbols() {
  // adds all symbols from fetched symbol list into the dropdown.
...
async function convCurrency(target, base, amount) { //function takes arguements and converts them, then displays on corresponding element on HTML
...
async function getHistorical(base, target) { //function takes arguements and adds to HTTPS link, then displays on corresponding element on HTML
  let today = new Date(); 
  ...
  let result = await getAPI(
    `https://api.apilayer.com/exchangerates_data/timeseries?start_date=${formatedNinety}&end_date=${formatedToday}&base=${base}&symbols=${target}`
  );
  ...
  for (let i = 0; i < rateKeys.length; i++) {
    //loops through the array created above, to print out the keys and values
    ...
    for (const curr in current) {
      ...
      let textNode = document.createTextNode(
        `On ${date}, if you had 1 ${base} it would be worth ${value} ${target}`
      );
      newElement.appendChild(textNode);
...
function createFaves() {    //Creates buttons based on current selection and adds them to HTML
  let base = document.querySelector("#base-currency");
  let baseCurr = base.value;
